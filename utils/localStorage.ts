...
export const readTasks = () => JSON.parse(window.localStorage.getItem(LOCAL_STORAGE_NAME)!) || [];
...
export const updateTaskInLocalStorage = (taskId: number, property: TaskProperties, value: any): ListItems => {
    const tasks = readTasks()
    const index = taskId - 1
    const editedTaskArray = [...tasks]
    const editedTask = { ...editedTaskArray[index], [property]: value }
    editedTaskArray[index] = editedTask
    window.localStorage.setItem(LOCAL_STORAGE_NAME, JSON.stringify(editedTaskArray));
